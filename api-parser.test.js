...
test('Извлечение ID категории из корректного URL', () => {
    const result = extractCategoryId('https://www.vprok.ru/catalog/7382/pomidory-i-ovoschnye-nabory');
...
test('Извлечение ID из URL с параметрами', () => {
    const result = extractCategoryId('https://www.vprok.ru/catalog/7382/pomidory-i-ovoschnye-nabory?page=2');
    assert.strictEqual(result.categoryId, '7382');
    assert.strictEqual(result.slug, 'pomidory-i-ovoschnye-nabory');
});
...
test('Форматирование товара без скидки', () => {
    ...
    assert.strictEqual(formatted.link, 'https://www.vprok.ru/product/tomaty-cherri-flamenko-krasnye-kruglye--1293724');
...
test('Форматирование товара со скидкой', () => {
    ...
    assert.strictEqual(formatted.name, 'Помидоры Фламенко сливовидные красные 1кг');
