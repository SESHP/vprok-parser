...
async function parseVprok(categoryUrl) {
    ...
    if (!urlMatch) {
        throw new Error('Неверный формат URL. Ожидается: https://www.vprok.ru/catalog/{id}/{slug}');
    }
    ...
    const result = await page.evaluate(async (apiUrl, categoryId, categorySlug) => {
    ...
    }, apiUrl, categoryId, categorySlug);
...
if (!url) {
    ...
    console.log('Пример: node api-parser.js https://www.vprok.ru/catalog/7382/pomidory-i-ovoschnye-nabory');
    process.exit(1);
}
